...
write.table(fullPS,file="rma.fullTrans.csv",sep=",",row.names=FALSE,col.names=TRUE)

source('~/LXSExonData/Programs/ComBat.R')
ComBat('rma.fullTrans.csv','sampleInfo.txt',skip=1,type='csv')
...
write.table(headLines,file="rma.fullTrans.LXS.PhenoGen.txt",quote=FALSE,row.names=FALSE,col.names=FALSE,sep="\t")
write.table(cbind(probeset_id = rownames(adjusted),adjusted),file="rma.fullTrans.LXS.PhenoGen.txt",sep="\t",row.names=FALSE,quote=FALSE,append=TRUE)

write.table(headLines.dabg,file="dabg.fullTrans.LXS.PhenoGen.txt",quote=FALSE,row.names=FALSE,col.names=FALSE,sep="\t")
write.table(cbind(probeset_id = rownames(DABG),DABG),file="dabg.fullTrans.LXS.PhenoGen.txt",sep="\t",row.names=FALSE,quote=FALSE,append=TRUE)
